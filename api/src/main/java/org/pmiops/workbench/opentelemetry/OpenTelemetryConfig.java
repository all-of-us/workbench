...
public class OpenTelemetryConfig {
  ...
  public SdkMeterProvider metricReader() {
    if (IS_GAE) {
      ...
      return SdkMeterProvider.builder().registerMetricReader(PeriodicMetricReader.create(GoogleCloudMetricExporter.createWithDefaultConfiguration())).build();
    } else {
      LOGGER.warning("Disable MetricReader, this should only happen in local dev environment.");
      return SdkMeterProvider.builder().registerMetricReader(PeriodicMetricReader.create(new NoopMetricExporter())).build();
    }
  }
