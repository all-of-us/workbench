...
public class CloudResourceManagerServiceImpl implements CloudResourceManagerService {
  ...
  public CloudResourceManagerServiceImpl(
      @Qualifier(Constants.CLOUD_RESOURCE_MANAGER_ADMIN_CREDS) Provider<GoogleCredential> cloudResourceManagerAdminCredsProvider,
  ...
  public List<Project> getAllProjectsForUser(User user) {
    try {
      return retryHandler.runAndThrowChecked((context) ->
          getCloudResourceManagerServiceWithImpersonation(user).projects().list().execute().getProjects());
    } catch (IOException e) {
