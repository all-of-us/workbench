...
public interface CloudBillingClient {
   ...
   *
   * <p>This is needed because we use Terra to update billing accounts, and in Terra's model, a
   * billing project can be associated to many workspaces. So it is an asynchronous process in
   * Terra. Terra has background job that runs every 5s to update billing account for all workspaces.
   * We have to poll this from Google to verify the operation succeeds.
   */
