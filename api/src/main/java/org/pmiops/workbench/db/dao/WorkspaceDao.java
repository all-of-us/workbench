...
public interface WorkspaceDao extends CrudRepository<DbWorkspace, Long>, WorkspaceDaoCustom {
  ...
  @Query(
      ...
      value =
          "SELECT workspace.workspace_id AS dbId, workspace.name, workspace.workspace_namespace AS namespace, "
              + "workspace.google_project AS googleProject, workspace.active_status AS activeStatus, workspace.billing_migration_status AS migrationStatus, "
              + "workspace.creator_id AS creatorId, workspace.creation_time AS creationTime, workspace.last_modified_time AS lastModifiedTime, "
  ...
  List<FieldsForOrphanChecking> getFieldsForOrphanChecking();
  ...
  interface FieldsForOrphanChecking {
    ...
    default boolean getMigrationStatusBool() {
      return getMigrationStatus() == 1; // NEW - the only valid status post-migration
    }

    default String logString() {
      return String.format(
          "WS: name %s, namespace %s, googProj %s, tier %s, status %s",
          getName(), getNamespace(), getGoogleProject(), getTierName(), getActiveStatusEnum());
    }
  }
}
