...
public class MeasurementQueryBuilder extends AbstractQueryBuilder {
  ...
  private static final String MEASUREMENT_SQL_TEMPLATE =
      "select distinct person_id, entry_date, concept_id\n"
          ...
          + "`\n"
          + "where\n";
  private static final String CONCEPT_ID_EQUAL_TEMPLATE = "concept_id = ${conceptId}";
  private static final String CONCEPT_ID_IN_TEMPLATE = "concept_id in unnest(${conceptIds})";
  ...
  public String buildQuery(
    ...
    for (SearchParameter parameter : searchGroupItem.getSearchParameters()) {
      ...
      for (Attribute attribute : parameter.getAttributes()) {
        ...
        String queryPartSql =
            CONCEPT_ID_EQUAL_TEMPLATE.replace("${conceptId}", "@" + namedParameter);
        if (AttrName.NUM.equals(attribute.getName())) {
          queryParts.add(
              processNumericalSql(
                  queryParams, "(" + queryPartSql + VALUE_AS_NUMBER + ")\n", attribute));
        } else if (AttrName.CAT.equals(attribute.getName())) {
          queryParts.add(
              processCategoricalSql(
                  queryParams, "(" + queryPartSql + VALUE_AS_CONCEPT_ID + ")\n", attribute));
        }
      }
    }
    ...
    String conceptIdSql =
        String.join(OR, queryParts).replace("${conceptIds}", "@" + idParameter)
  ...
  private void validateSearchParameter(SearchParameter param) {
    from(typeBlank().or(measTypeInvalid()))
        .test(param)
  ...
  private String processNumericalSql(
      Map<String, QueryParameterValue> queryParams, String baseSql, Attribute attribute) {
  ...
  private String processCategoricalSql(
      Map<String, QueryParameterValue> queryParams, String baseSql, Attribute attribute) {
    String namedParameter1 =
        addQueryParameterValue(
            queryParams,
