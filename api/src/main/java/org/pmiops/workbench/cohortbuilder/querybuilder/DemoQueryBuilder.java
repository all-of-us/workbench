...
/**
 * DemoQueryBuilder builds SQL for BigQuery for the following criteria types: DEMO_GEN, DEMO_AGE,
...
public class DemoQueryBuilder extends AbstractQueryBuilder {
  ...
  private static final String DEMO_GEN = "p.gender_concept_id in unnest(${gen})\n";
  ...
  private static final String DEMO_RACE = "p.race_concept_id in unnest(${race})\n";
  ...
  private static final String DEMO_ETH = "p.ethnicity_concept_id in unnest(${eth})\n";
  ...
  public String buildQuery(
    ...
    from(containsAgeAndDec())
        .test(searchGroupItem.getSearchParameters())
        .throwException(AGE_DEC_MESSAGE);
    ListMultimap<TreeSubType, Object> paramMap =
    ...
    for (TreeSubType key : paramMap.keySet()) {
      ...
      switch (key) {
        case GEN:
          Long[] demoIds =
              paramMap.get(key).stream()
                  .filter(Long.class::isInstance)
                  .map(Long.class::cast)
                  .toArray(Long[]::new);
          String genParameter =
              addQueryParameterValue(queryParams, QueryParameterValue.array(demoIds, Long.class));
        ...
        case RACE:
          Long[] raceIds =
              paramMap.get(key).stream()
                  ...
                  .map(Long.class::cast)
                  .toArray(Long[]::new);
          String raceParameter =
              addQueryParameterValue(queryParams, QueryParameterValue.array(raceIds, Long.class));
        ...
        case AGE:
          ...
          queryParts.add(
              DEMO_AGE.replace("${operator}", OperatorUtils.getSqlOperator(attribute.getOperator()))
        ...
        case ETH:
          Long[] ethIds =
              paramMap.get(key).stream()
                  ...
                  .toArray(Long[]::new);
          String ethParameter =
              addQueryParameterValue(queryParams, QueryParameterValue.array(ethIds, Long.class));
    ...
    param
        ...
        .forEach(
            attr -> {
              ...
              from(operatorNull())
                  .test(attr)
                  .throwException(NOT_VALID_MESSAGE, ATTRIBUTE, OPERATOR, oper);
              from(operandsEmpty()).test(attr).throwException(EMPTY_MESSAGE, OPERANDS);
              from(notBetweenOperator().and(operandsNotOne()))
                  .test(attr)
                  .throwException(ONE_OPERAND_MESSAGE, ATTRIBUTE, name, oper);
              from(betweenOperator().and(operandsNotTwo()))
                  .test(attr)
                  .throwException(TWO_OPERAND_MESSAGE, ATTRIBUTE, name, oper);
              from(operandsNotNumbers())
                  .test(attr)
                  .throwException(OPERANDS_NUMERIC_MESSAGE, ATTRIBUTE, name);
            });
  }

  protected ListMultimap<TreeSubType, Object> getMappedParameters(
    ...
    for (SearchParameter parameter : searchParameters) {
      ...
      if (parameter.getSubtype().equals(TreeSubType.AGE.name())) {
        ...
        mappedParameters.put(
            TreeSubType.AGE,
            parameter.getAttributes().isEmpty() ? null : parameter.getAttributes().get(0));
      } else if (parameter.getSubtype().equals(TreeSubType.DEC.name())) {
        mappedParameters.put(TreeSubType.DEC, parameter.getValue());
      } else {
        mappedParameters.put(
            TreeSubType.fromValue(parameter.getSubtype()), parameter.getConceptId());
      }
    }
