...
public class PPIQueryBuilder extends AbstractQueryBuilder {
  ...
  private static final String QUESTION_ANSWER_IN_CLAUSE = "in (${conceptId}) ";
  ...
  public String buildQuery(
    ...
    for (SearchParameter parameter : searchGroupItem.getSearchParameters()) {
      from(typeBlank().or(ppiTypeInvalid()))
          .test(parameter)
      ...
      if (parameter.getConceptId() == null && parameter.getGroup()) {
        ...
        sqlTemplate.append(SURVEY_IN_CLAUSE.replace("${subtype}", "@" + subtype));
      } else {
        from(conceptIdNull())
            .test(parameter)
        ...
        sqlTemplate.append(
            QUESTION_ANSWER_IN_CLAUSE.replace("${conceptId}", "@" + namedParameterConceptId));
        if (!parameter.getGroup()) {
          ...
          String namedParameter =
              addQueryParameterValue(
                  queryParams, QueryParameterValue.int64(new Long(attr.getOperands().get(0))));
          sqlTemplate.append(
              isValueAsNum
  ...
  private void validateAttribute(Attribute attr) {
    ...
    from(operandsNotOne())
        .test(attr)
        .throwException(ONE_OPERAND_MESSAGE, ATTRIBUTE, name, Operator.EQUAL);
