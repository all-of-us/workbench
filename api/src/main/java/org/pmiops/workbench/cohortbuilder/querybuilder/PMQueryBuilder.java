...
public class PMQueryBuilder extends AbstractQueryBuilder {
  ...
  private static final String BP_INNER_SQL_TEMPLATE =
      "select person_id, entry_date from `${projectId}.${dataSetId}."
          + TABLE_ID
          + "`\n"
          + "where concept_id = ${conceptId}\n";

  private static final String BP_SQL_TEMPLATE = "select person_id from( ${bpInnerSqlTemplate} )\n";

  private static final String BASE_SQL_TEMPLATE =
      "select person_id from `${projectId}.${dataSetId}."
          + TABLE_ID
          + "`\n"
          + "where concept_id = ${conceptId}\n"
          + "and is_standard = 0\n";

  private static final String VALUE_AS_NUMBER_SQL_TEMPLATE = BASE_SQL_TEMPLATE + VALUE_AS_NUMBER;
  ...
  public String buildQuery(
    ...
    for (SearchParameter parameter : searchGroupItem.getSearchParameters()) {
      ...
      if (parameter.getAttributes().isEmpty()) {
        ...
        String namedParameterConceptId =
      ...
      for (Attribute attribute : parameter.getAttributes()) {
        ...
        if (AttrName.ANY.equals(attribute.getName())) {
          ...
          String namedParameterConceptId =
              addQueryParameterValue(
                  queryParams, QueryParameterValue.int64(attribute.getConceptId()));
          tempQueryParts.add(tempSql.replace("${conceptId}", "@" + namedParameterConceptId));
        } else {
          ...
          String numberOrConceptIdSql =
              isValueAsNumber ? VALUE_AS_NUMBER_SQL_TEMPLATE : VALUE_AS_CONCEPT_ID_SQL_TEMPLATE;
          String tempSql = isBP ? BP_INNER_SQL_TEMPLATE + VALUE_AS_NUMBER : numberOrConceptIdSql;
          String namedParameterConceptId =
              addQueryParameterValue(
                  queryParams,
                  QueryParameterValue.int64(
          ...
          String namedParameter1 =
              isValueAsNumber
                  ...
                  : addQueryParameterValue(
                      queryParams,
                      QueryParameterValue.int64(new Integer(attribute.getOperands().get(0))));
      ...
      if (isBP) {
        queryParts.add(
            BP_SQL_TEMPLATE.replace(
                "${bpInnerSqlTemplate}", String.join(INTERSECT_DISTINCT, tempQueryParts)));
      } else {
  ...
  private void validateSearchParameter(SearchParameter param) {
    ...
    from(typeBlank().or(pmTypeInvalid()))
        .test(param)
        .throwException(NOT_VALID_MESSAGE, PARAMETER, TYPE, type);
    from(subtypeBlank().or(pmSubtypeInvalid()))
        .test(param)
        .throwException(NOT_VALID_MESSAGE, PARAMETER, SUBTYPE, subtype);
    if (param.getSubtype().equals(TreeSubType.BP.name())) {
      ...
      from(conceptIdNull())
          .test(param)
          .throwException(NOT_VALID_MESSAGE, PARAMETER, CONCEPT_ID, conceptId);
    }
  }
  ...
  private void validateAttribute(Attribute attr, boolean isBP) {
    ...
    if (!isBP) {
      ...
      from(operatorNull().and(conceptIdIsNull()))
          .test(attr)
