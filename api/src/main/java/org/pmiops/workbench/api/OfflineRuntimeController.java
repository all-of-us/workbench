...
public class OfflineRuntimeController implements OfflineRuntimeApiDelegate {
  ...
  public ResponseEntity<Void> checkRuntimes() {
    ...
    final Duration maxAge = Duration.ofDays(config.firecloud.leoRuntimeMaxAgeDays);
    final Duration idleMaxAge = Duration.ofDays(config.firecloud.leoRuntimeIdleMaxAgeDays);
