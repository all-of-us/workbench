...
import static org.pmiops.workbench.cohortbuilder.util.QueryParameterValues.TIMESTAMP_FORMATTER;
...
public enum WorkspaceParameter implements QueryParameterColumn<ReportingWorkspace> {
  ...
  CREATION_TIME(
      "creation_time",
      w -> TIMESTAMP_FORMATTER.format(Instant.ofEpochMilli(w.getCreationTime())),
      DowncastObject.TIMESTAMP_STRING);
