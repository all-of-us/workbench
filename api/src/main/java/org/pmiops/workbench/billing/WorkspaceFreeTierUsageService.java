...
public class WorkspaceFreeTierUsageService {
    ...
    private static final Logger logger = Logger.getLogger(WorkspaceFreeTierUsageService.class.getName());

    @Autowired
    public WorkspaceFreeTierUsageService(WorkspaceFreeTierUsageDao workspaceFreeTierUsageDao, WorkspaceDao workspaceDao) {
    ...
    public void updateWorkspaceFreeTierUsageInDB(List<WorkspaceDao.WorkspaceCostView> dbCost, Map<Long, Double> liveCostByWorkspace) {
        ...
        final List<Long> workspacesIdsToUpdate =
                liveCostByWorkspace.keySet()
                        .stream()
                        .filter(currentId ->
                                CostComparisonUtils.compareCosts(dbCostByWorkspace.get(currentId), liveCostByWorkspace.get(currentId)) != 0)
                        .collect(
                                Collectors.toList());
