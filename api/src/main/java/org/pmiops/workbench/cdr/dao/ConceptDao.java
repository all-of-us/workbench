...
public interface ConceptDao extends CrudRepository<Concept, Long> {
    ...
    @Query(value = "select c.* from cdr.concept c where c.concept_name like %:conceptName% and c.domain_id in ('Condition','Observation','Procedure', 'Measurement', 'Drug') order by c.count_value desc limit 10;",
            nativeQuery = true)
    List<Concept> findConceptLikeName(@Param("conceptName") String conceptName);

    @Query(value = "select c.* from cdr.concept c where c.domain_id in ('Condition','Observation','Procedure', 'Measurement', 'Drug') order by c.count_value desc limit 10;",
    nativeQuery = true)
