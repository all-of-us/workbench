...
//  }
  ...
  public void initializeFunctionRegistry(QueryEngine queryEngine) {
    ...
    functionRegistry.registerPattern("match", "match(?1) against  (?2 in boolean mode)", basicTypeRegistry.resolve(StandardBasicTypes.DOUBLE));
    functionRegistry.registerPattern("matchConcept", "match(?1, ?2, ?3, ?4) against (?5 in boolean mode)", basicTypeRegistry.resolve(StandardBasicTypes.DOUBLE));
