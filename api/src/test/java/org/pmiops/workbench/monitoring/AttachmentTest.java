...
public class AttachmentTest {
  ...
  public void testBufferEntryStatus() {
    assertThat(Attachment.BUFFER_ENTRY_STATUS.getAllowedMetrics()).contains(GaugeMetric.BILLING_BUFFER_PROJECT_COUNT);
    assertThat(Attachment.BUFFER_ENTRY_STATUS.supportsMetric(GaugeMetric.COHORT_COUNT)).isFalse();
    assertThat(Attachment.BUFFER_ENTRY_STATUS.supportsDiscreteValue(BufferEntryStatus.AVAILABLE.toString())).isTrue();
  ...
  public void testUnrestricted() {
    ...
    assertThat(Arrays.stream(GaugeMetric.values()).allMatch(
        Attachment.DEBUG_CONTINUOUS_VALUE::supportsMetric));
  }
}
