...
public class MeasurementBundleTest extends SpringTest {
  ...
  public void testBuild_missingMeasurementsThrows() {
      assertThrows(IllegalStateException.class, () -> {
          MeasurementBundle.builder().addTag(MetricLabel.USER_DISABLED, Boolean.valueOf(true).toString()).build();
      });
  }
  ...
  public void testBuild_incompatibleMetricAndAttachmentThrows() {
      assertThrows(IllegalStateException.class, () -> {
          MeasurementBundle.builder().addMeasurement(GaugeMetric.COHORT_COUNT, 101L).addMeasurement(GaugeMetric.BILLING_BUFFER_PROJECT_COUNT, 202L).addTag(MetricLabel.BUFFER_ENTRY_STATUS, BufferEntryStatus.AVAILABLE.toString()).build();
      });
  }
