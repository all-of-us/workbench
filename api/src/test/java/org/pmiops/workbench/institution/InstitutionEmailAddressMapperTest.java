...
public class InstitutionEmailAddressMapperTest extends SpringTest {
  ...
  public void test_modelToDb() {
    ...
    final List<String> ctRawAddresses =
    ...
    final Set<DbInstitutionEmailAddress> rtDbAddresses = mapper.modelToDb(modelInst, dbInst, REGISTERED_ACCESS_TIER);
    final Set<DbInstitutionEmailAddress> ctDbAddresses = mapper.modelToDb(modelInst, dbInst, CONTROLLED_ACCESS_TIER);
    ...
    final List<TierEmailAddresses> roundTripModelAddresses = mapper.dbAddressesToTierEmailAddresses(Sets.union(rtDbAddresses, ctDbAddresses));
  ...
  public void test_modelToDb_tierNotFound() {
    TierEmailAddresses rtEmailAddresses = new TierEmailAddresses().emailAddresses(ImmutableList.of()).accessTierShortName(RT_SHORT_NAME);
    final Institution modelInst =
        new Institution()
            .shortName("Broad")
            .displayName("The Broad Institute")
  ...
  public void test_dbAddressesToModel() {
    ...
    final Set<DbInstitutionEmailAddress> dbAddresses =
        Sets.newHashSet(
            ...
            new DbInstitutionEmailAddress()
                ...
                .setInstitution(dbInst).setAccessTier(REGISTERED_ACCESS_TIER),
            new DbInstitutionEmailAddress()
                .setEmailAddress("joel@broad.org")
                .setInstitution(dbInst).setAccessTier(REGISTERED_ACCESS_TIER),
    new DbInstitutionEmailAddress()
        .setEmailAddress("foo@broad.org")
        .setInstitution(dbInst).setAccessTier(CONTROLLED_ACCESS_TIER),
            new DbInstitutionEmailAddress()
                .setEmailAddress("foo@broad.org")
                .setInstitution(dbInst).setAccessTier(CONTROLLED_ACCESS_TIER)
        );

    final List<String> expectedRt =
       ImmutableList.of("eric@broad.org", "joel@broad.org");
    final List<String> expectedCt =
        ImmutableList.of("foo@broad.org");

    final List<TierEmailAddresses> modelTierAddresses = mapper.dbAddressesToTierEmailAddresses(dbAddresses);
    TierEmailAddresses expectedRtTierEmailAddress = new TierEmailAddresses().accessTierShortName(RT_SHORT_NAME).emailAddresses(expectedRt);
    TierEmailAddresses expectedCtTierEmailAddress = new TierEmailAddresses().accessTierShortName(CT_SHORT_NAME).emailAddresses(expectedCt);
    assertThat(modelTierAddresses).containsExactly(expectedRtTierEmailAddress, expectedCtTierEmailAddress);
    ...
    final Institution modelInst =
        new Institution()
            ...
            .tierEmailAddresses(ImmutableList.of(expectedRtTierEmailAddress, expectedCtTierEmailAddress));

    final Set<DbInstitutionEmailAddress> roundTripRtDbAddresses = mapper.modelToDb(modelInst, dbInst, REGISTERED_ACCESS_TIER);
    final Set<DbInstitutionEmailAddress> roundTripCtDbAddresses = mapper.modelToDb(modelInst, dbInst, CONTROLLED_ACCESS_TIER);
