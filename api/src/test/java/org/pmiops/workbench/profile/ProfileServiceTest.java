...
public class ProfileServiceTest extends SpringTest {
  ...
  @MockBean
  private InstitutionDao mockInstitutionDao;
  @MockBean
  private InstitutionService mockInstitutionService;
  @MockBean
  private UserService mockUserService;
  @MockBean
  private UserTermsOfServiceDao mockUserTermsOfServiceDao;
  ...
  public void validateInstitutionalAffiliation_noRole() {
      assertThrows(BadRequestException.class, () -> {
          VerifiedInstitutionalAffiliation affiliation = new VerifiedInstitutionalAffiliation().institutionShortName("Broad").institutionDisplayName("The Broad Institute");
  ...
  public void validateInstitutionalAffiliation_noOtherText() {
      assertThrows(BadRequestException.class, () -> {
          VerifiedInstitutionalAffiliation affiliation = new VerifiedInstitutionalAffiliation().institutionShortName("Broad").institutionDisplayName("The Broad Institute").institutionalRoleEnum(InstitutionalRole.OTHER);
          Profile profile = new Profile().verifiedInstitutionalAffiliation(affiliation);
          when(mockInstitutionDao.findOneByShortName("Broad")).thenReturn(Optional.of(BROAD_INSTITUTION));
          profileService.validateAffiliation(profile);
      });
  ...
  public void validateInstitutionalAffilation_badEmail() {
      assertThrows(BadRequestException.class, () -> {
          VerifiedInstitutionalAffiliation affiliation = new VerifiedInstitutionalAffiliation().institutionShortName("Broad").institutionDisplayName("The Broad Institute").institutionalRoleEnum(InstitutionalRole.OTHER).institutionalRoleOtherText("Kibitzing");
          Profile profile = new Profile().verifiedInstitutionalAffiliation(affiliation).contactEmail("kibitz@broadinstitute.org");
          when(mockInstitutionDao.findOneByShortName("Broad")).thenReturn(Optional.of(BROAD_INSTITUTION));
          when(mockInstitutionService.validateAffiliation(any(DbVerifiedInstitutionalAffiliation.class), anyString())).thenReturn(false);
          DbVerifiedInstitutionalAffiliation dbVerifiedInstitutionalAffiliation = new DbVerifiedInstitutionalAffiliation();
          dbVerifiedInstitutionalAffiliation.setInstitution(BROAD_INSTITUTION);
          when(mockVerifiedInstitutionalAffiliationMapper.modelToDbWithoutUser(affiliation, mockInstitutionService)).thenReturn(dbVerifiedInstitutionalAffiliation);
          profileService.validateAffiliation(profile);
      });
  }
  ...
  public void validateProfile_usernameChanged_admin() {
      assertThrows(BadRequestException.class, () -> {
          ...
          when(mockUserService.hasAuthority(loggedInUser.getUserId(), Authority.ACCESS_CONTROL_ADMIN)).thenReturn(true);
          profileService.validateProfile(new Profile().username("new"), new Profile().username("old"));
      });
  }
  ...
  public void validateProfile_usernameTooLong() {
      assertThrows(BadRequestException.class, () -> {
          profileService.validateProfile(new Profile().username(StringUtils.repeat("asdf", 30)), new Profile());
      });
  }
  ...
  public void validateProfile_contactEmailChanged_user() {
      assertThrows(BadRequestException.class, () -> {
          ...
          profileService.validateProfile(new Profile().contactEmail("new@gmail.com"), new Profile().contactEmail("old@gmail.com"));
      });
  }
  ...
  public void validateProfile_addressRemoveZipCode() {
      assertThrows(BadRequestException.class, () -> {
          ...
          Address oldAddress = new Address().streetAddress1("asdf").city("asdf").state("asdf").country("asdf").zipCode("asdf");
          Address newAddress = new Address().streetAddress1("asdf").city("asdf").state("asdf").country("asdf");
          profileService.validateProfile(new Profile().address(newAddress), new Profile().address(oldAddress));
      });
  }
  ...
  public void validateProfile_noInstitutionMatch() {
      assertThrows(BadRequestException.class, () -> {
          Profile newProfile = new Profile().contactEmail("not-a-match@gmail.com").verifiedInstitutionalAffiliation(VALID_PROFILE.getVerifiedInstitutionalAffiliation());
          when(mockInstitutionDao.findOneByShortName("Broad")).thenReturn(Optional.of(BROAD_INSTITUTION));
