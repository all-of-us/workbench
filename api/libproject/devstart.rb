...
SERVICES = %w[servicemanagement.googleapis.com storage-component.googleapis.com iam.googleapis.com
              ...
              clouderrorreporting.googleapis.com bigquery-json.googleapis.com].freeze
DRY_RUN_CMD = %w{echo [DRY_RUN]}.freeze
...
def get_config(env)
  unless ENVIRONMENTS.fetch(env, {}).key?(:config_json)
    raise ArgumentError, "env '#{env}' lacks a valid configuration"
  end

  ENVIRONMENTS[env][:config_json]
end

def get_featured_workspaces_config(env)
  unless ENVIRONMENTS.fetch(env, {}).key?(:featured_workspaces_json)
    raise ArgumentError, "env '#{env}' lacks a valid featured workspaces configuration"
  end

  ENVIRONMENTS[env][:featured_workspaces_json]
end

def get_cdr_versions_file(project)
  unless ENVIRONMENTS.fetch(project, {}).key?(:cdr_versions_json)
    raise ArgumentError, "project #{project} lacks a valid env configuration"
  end

  ENVIRONMENTS[project][:cdr_versions_json]
end

def get_cdr_sql_project(project)
  unless ENVIRONMENTS.fetch(project, {}).key?(:cdr_sql_instance)
    raise ArgumentError, "project #{project} lacks a valid env configuration"
  end

  ENVIRONMENTS[project][:cdr_sql_instance].split(':')[0]
end

def get_gae_vars(project)
  unless ENVIRONMENTS.fetch(project, {}).key?(:gae_vars)
    raise ArgumentError, "project #{project} lacks GAE env variables"
  end

  ENVIRONMENTS[project][:gae_vars]
end
...
def run_gradle(cmd_name, args)
  ...
  begin
    ...
    if $ERROR_INFO && $ERROR_INFO.status != 0
...
def run_with_redirects(command_string, to_redact = '')
  ...
  command_to_echo.sub! to_redact, '*' * to_redact.length if to_redact
  common.put_command(command_to_echo)
  raise('Error running: ' + command_to_echo) unless system(command_string)
end

def get_auth_login_account
  `gcloud config get-value account`.strip
end
...
def backfill_billing_project_users(cmd_name, *args)
  ...
  common.status 'DRY RUN -- CHANGES WILL NOT BE PERSISTED' if op.opts.dry_run
  ...
  flags += ['--dry-run'] if op.opts.dry_run
...
def authority_options(cmd_name, args)
  ...
  op.add_validator ->(opts) { raise ArgumentError unless opts.email && opts.authority }
  op
end
...
def load_es_index(cmd_name, *args)
  ...
  op.add_validator ->(opts) { raise ArgumentError unless ENVIRONMENTS.key? opts.env }
  ...
  ServiceAccountContext.new((auth_project || TEST_PROJECT)).run do
...
def deploy_gcs_artifacts(cmd_name, args)
  ...
  Dir.chdir('cluster-resources') do
    ...
    # environment setup
    if !auth_domain_group.nil? && !auth_domain_group.empty?
      run_inline_or_log(op.opts.dry_run, %W[
...
def deploy_app(cmd_name, args, with_cron, with_gsuite_admin)
  ...
  promote = '--no-promote'
  promote = if op.opts.promote.nil?
              op.opts.version ? '--no-promote' : '--promote'
            else
...
def get_fc_config(project)
  ...
  JSON.parse(File.read("config/#{config_json}"))['firecloud']
end

def get_leo_api_url(project)
  get_fc_config(project)['leoBaseUrl']
end

def get_auth_domain(project)
  get_fc_config(project)['registeredDomainName']
end

def get_auth_domain_group(project)
  get_fc_config(project)['registeredDomainGroup']
end

def get_firecloud_base_url(project)
  get_fc_config(project)['baseUrl']
end

def get_es_base_url(env)
  config_json = get_config(env)
  JSON.parse(File.read("config/#{config_json}"))['elasticsearch']['baseUrl']
end

def load_config(project, dry_run = false)
  config_json = get_config(project)
  featured_workspaces_config = get_featured_workspaces_config(project)
  unless config_json && featured_workspaces_config
...
def print_scoped_access_token(cmd_name, args)
  ...
  gcc.validate
  ServiceAccountContext.new(gcc.project).run do
    op.opts.scopes = [] if op.opts.scopes.nil?
