...
def get_iam_user_vars(project)
  iam_user = ENVIRONMENTS.fetch(project, {})[:cloud_sql_iam_user]
  if iam_user
    return {
      'CLOUD_SQL_IAM_USER' => iam_user,
      'CDR_CLOUD_SQL_IAM_USER' => iam_user
    }
  end
  return {}
end
...
def deploy_app(cmd_name, args, with_cron, with_queue)
  ...
  ENV.update(get_iam_user_vars(gcc.project))
