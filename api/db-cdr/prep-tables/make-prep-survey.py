...
headersAll = ['id', 'parent_id', 'code', 'name', 'type', 'min', 'max',
              ...
              'registered_answer_bucketed', 'registered_answer_suppressed', 'controlled_topic_suppressed',
              'controlled_question_suppressed', 'controlled_answer_bucketed', 'controlled_answer_suppressed']
            ...
            if process_row(row):
                ...
                if row_number == 1:
                    ...
                    write_row(controlled_writer, 0, question_code, question_label,
                              'SURVEY', None, None, item_flags, 'controlled', None)
                ...
                if len(row['Section Header']) > 0:
                    ...
                    if position == -1:
                        ...
                        header = row['Section Header'][:position].replace("Thanks for your answers.", "")
                    ...
                    if item_flags["CONTROLLED_TOPIC_SUPPRESSED"] != 1:
                        ...
                        write_row(controlled_writer, survey_id_controlled, topic_code, header
                                  , 'TOPIC', None, None, item_flags, 'controlled', long_code_to_short_code)
                ...
                add_all_answers(all_writer, question_code, choices, question_id_all, None,
                ...
                if item_flags["REGISTERED_QUESTION_SUPPRESSED"] != 1:
                    ...
                    write_row(registered_writer, parent_id_registered, question_code, question_label
                              , 'QUESTION', None, None, item_flags, 'registered', long_code_to_short_code)
...
def write_row(writer, parent_id, code, name, item_type, min_value, max_value,
    ...
    if item_type == 'QUESTION':
        dict_row_register['code'] = get_short_code(code, long_code_to_short_code)
...
def add_all_answers(writer, question_code, choices, parent_id, min_value, max_value, flags, long_code_to_short_code):
    if choices != '':
...
def add_answers(writer, choices, question_code, parent_id, min_value, max_value, flags, file_type,
    ...
    if choices != '':
        ...
        for answer in answers:
            ...
            if file_type == 'controlled':
                if answer_suppression.get("CONTROLLED_ANSWER_SUPPRESSED") is None \
                        or (answer_suppression.get("CONTROLLED_ANSWER_SUPPRESSED") is not None
                            and code not in answer_suppression.get("CONTROLLED_ANSWER_SUPPRESSED")):
                    write_row(writer, parent_id, code, item_array[1].strip()
                              , 'ANSWER', min_value, max_value, flags, file_type, None)

            if file_type == 'registered':
                if answer_suppression.get("REGISTERED_ANSWER_SUPPRESSED") is None \
                        or (answer_suppression.get("REGISTERED_ANSWER_SUPPRESSED") is not None
                            and code not in answer_suppression.get("REGISTERED_ANSWER_SUPPRESSED")):
                    write_row(writer, parent_id, code, item_array[1].strip()
                              , 'ANSWER', min_value, max_value, flags, file_type, None)
