...
# Current year for age decile calculation
cur_year=$(date +"%Y")
...
# Person gender identity table has person_id, gender_concept_id, and gi_concept_id (gender identity)
# The gender identity concept is from their answer to the gi question concept
...
# 6	Number of persons by age decile (age deciles 3 +)
echo "Getting age decile count"
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`${WORKBENCH_PROJECT}.${WORKBENCH_DATASET}.achilles_results\` (id, analysis_id, stratum_1, count_value,source_count_value)
select 0, 6 as analysis_id,  CAST(floor((${cur_year} - year_of_birth)/10) AS STRING) as stratum_2,
COUNT(distinct person_id) as count_value, 0 as source_count_value
from \`${BQ_PROJECT}.${BQ_DATASET}.person\`
where floor((${cur_year} - year_of_birth)/10) >=3
group by stratum_2"

# Age decile 2 (18-29 yrs old) count
bq --quiet --project=$BQ_PROJECT query --nouse_legacy_sql \
"insert into \`${WORKBENCH_PROJECT}.${WORKBENCH_DATASET}.achilles_results\` (id, analysis_id, stratum_1, count_value,source_count_value)
select 0, 6 as analysis_id,  '2' as stratum_2,
COUNT(distinct person_id) as count_value, 0 as source_count_value
from \`${BQ_PROJECT}.${BQ_DATASET}.person\`
where ((${cur_year} - year_of_birth) > 18 and (${cur_year} - year_of_birth) < 30)"
...
"insert into \`${WORKBENCH_PROJECT}.${WORKBENCH_DATASET}.achilles_results\`
 ...
 select 0 as id,${COUNT_ANALYSIS_ID} as analysis_id,CAST(vo1.visit_concept_id AS STRING) as stratum_1,'Visit' as stratum_3,
...
"insert into \`${WORKBENCH_PROJECT}.${WORKBENCH_DATASET}.achilles_results\`
(id, analysis_id, stratum_1, stratum_3, count_value, source_count_value)
select 0, ${COUNT_ANALYSIS_ID} as analysis_id,
...
select 0, ${GENDER_ANALYSIS_ID} as analysis_id,
	...
	(select COUNT(distinct p2.PERSON_ID) from \`${WORKBENCH_PROJECT}.${WORKBENCH_DATASET}.${person_gi}\` p2 inner join
	  \`${BQ_PROJECT}.${BQ_DATASET}.condition_occurrence\` co2 on p2.person_id=co2.person_id
	  where co2.condition_source_concept_id=co1.condition_concept_id and p2.gender_concept_id = p1.gender_concept_id and
	   p2.gi_concept_id = p1.gi_concept_id) as source_count_value
...
select 0, 3102 as analysis_id,
  ...
  count(distinct p1.person_id) as count_value,(select COUNT(distinct p2.PERSON_ID) from \`${BQ_PROJECT}.${BQ_DATASET}.person\` p2 inner join \`${BQ_PROJECT}.${BQ_DATASET}.condition_occurrence\` co2 on p2.person_id=co2.person_id where co2.condition_source_concept_id=co1.condition_concept_id
  and (extract(year from co2.condition_start_date) - p2.year_of_birth) > 18 and (extract(year from co2.condition_start_date) - p2.year_of_birth) < 30) as source_count_value
from \`${BQ_PROJECT}.${BQ_DATASET}.person\` p1 inner join
\`${BQ_PROJECT}.${BQ_DATASET}.condition_occurrence\` co1
on p1.person_id = co1.person_id
...
select 0, 3101 as analysis_id,
	...
	COUNT(distinct p1.PERSON_ID) as count_value,
	(select COUNT(distinct co2.PERSON_ID) from \`${BQ_PROJECT}.${BQ_DATASET}.observation\` co2 join \`${BQ_PROJECT}.${BQ_DATASET}.person\` p2 on p2.person_id=co2.person_id where co2.observation_source_concept_id=co1.observation_concept_id and p2.gender_concept_id = p1.gender_concept_id) as source_count_value
from \`${BQ_PROJECT}.${BQ_DATASET}.person\` p1 inner join
...
join \`${WORKBENCH_PROJECT}.${WORKBENCH_DATASET}.survey_module\` sm on cr.concept_id_2 = sm.concept_id
Where (o.observation_source_concept_id > 0 and o.value_source_concept_id > 0)
Group by o.observation_source_concept_id,o.value_source_concept_id,o.value_as_string,sm.concept_id"
...
Group by o.observation_source_concept_id,o.value_as_number,sm.concept_id"
...
join  \`${WORKBENCH_PROJECT}.${WORKBENCH_DATASET}.survey_module\` sm on cr.concept_id_2=sm.concept_id where
(o.observation_source_concept_id > 0 and o.value_source_concept_id > 0)
group by sm.concept_id,o.observation_source_concept_id,o.value_source_concept_id,o.value_as_number,o.value_as_string,p.gender_concept_id"
...
group by sm.concept_id,o.observation_source_concept_id,o.value_as_number,p.gender_concept_id"
...
group by sm.concept_id,o.observation_source_concept_id,o.value_source_concept_id,o.value_as_string,stratum_5"
...
and (o.observation_source_concept_id != 1585966)
and floor((extract(year from o.observation_date) - p.year_of_birth)/10) >=3
group by sm.concept_id,o.observation_source_concept_id,o.value_as_number,stratum_5"
