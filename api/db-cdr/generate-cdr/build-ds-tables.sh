...
if [[ "$TABLE_LIST" == *'visit_detail'* ]];
then
  echo "ds_condition_occurrence (OMOP 5.3.1) - inserting data"
  ...
  "INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.ds_condition_occurrence\`
      ...
      STOP_REASON, VISIT_OCCURRENCE_ID, VISIT_OCCURRENCE_CONCEPT_NAME, VISIT_DETAIL_ID, CONDITION_SOURCE_VALUE, CONDITION_SOURCE_CONCEPT_ID,
...
if [[ "$TABLE_LIST" == *'visit_detail'* ]];
then
  echo "ds_drug_exposure (OMOP 5.3.1) - inserting data"
  bq --quiet --project_id=$BQ_PROJECT query --nouse_legacy_sql \
  "INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.ds_drug_exposure\`
      ...
      DAYS_SUPPLY, SIG, ROUTE_CONCEPT_ID, ROUTE_CONCEPT_NAME, LOT_NUMBER, VISIT_OCCURRENCE_ID, VISIT_OCCURRENCE_CONCEPT_NAME, VISIT_DETAIL_ID, DRUG_SOURCE_VALUE,
...
if [[ "$TABLE_LIST" == *'visit_detail'* ]];
then
  echo "ds_measurement (OMOP 5.3.1) - inserting data"
  bq --quiet --project_id=$BQ_PROJECT query --nouse_legacy_sql \
  "INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.ds_measurement\`
      ...
      RANGE_LOW, RANGE_HIGH, VISIT_OCCURRENCE_ID, VISIT_OCCURRENCE_CONCEPT_NAME, VISIT_DETAIL_ID, MEASUREMENT_SOURCE_VALUE,
...
if [[ "$TABLE_LIST" == *'visit_detail'* ]];
then
  echo "ds_observation (OMOP 5.3.1) - inserting data"
  bq --quiet --project_id=$BQ_PROJECT query --nouse_legacy_sql \
  "INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.ds_observation\`
      ...
      UNIT_CONCEPT_NAME, VISIT_OCCURRENCE_ID, VISIT_OCCURRENCE_CONCEPT_NAME, VISIT_DETAIL_ID, OBSERVATION_SOURCE_VALUE,
...
then
  ...
  "INSERT INTO \`$BQ_PROJECT.$BQ_DATASET.ds_procedure_occurrence\`
      ...
      QUANTITY, VISIT_OCCURRENCE_ID, VISIT_OCCURRENCE_CONCEPT_NAME, VISIT_DETAIL_ID, PROCEDURE_SOURCE_VALUE, PROCEDURE_SOURCE_CONCEPT_ID,
