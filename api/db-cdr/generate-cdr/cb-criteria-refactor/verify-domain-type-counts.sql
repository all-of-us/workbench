...
select * from (
                  select '01-sequential' run_type, domain_id, type, is_group, 'loinc-clinical-std-name' name, count(is_group) sum_grp_count
                       , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'CLIN' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '02-parallel' run_type, domain_id, type, is_group, 'loinc-clinical-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'CLIN' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '03-parallel-multi' run_type, domain_id, type, is_group, 'loinc-clinical-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'CLIN' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '10-original' run_type, domain_id, type, is_group, 'loinc-clinical-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'CLIN' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '20-std-src' run_type, domain_id, type, is_group, 'loinc-clinical-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'CLIN' and is_standard = 1 group by 4,5,3,2
              )
order by domain_id, type, is_group, name, run_type;

-- MEASUREMENT - Labs - STANDARD LOINC - SQL-ORDER = 15
select * from (
                  select '01-sequential' run_type, domain_id, type, is_group, 'loinc-labs-std-name' name, count(is_group) sum_grp_count
                       , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'LAB' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '02-parallel' run_type, domain_id, type, is_group, 'loinc-labs-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'LAB' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '03-parallel-multi' run_type, domain_id, type, is_group, 'loinc-labs-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'LAB' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '10-original' run_type, domain_id, type, is_group, 'loinc-labs-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'LAB' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '20-std-src' run_type, domain_id, type, is_group, 'loinc-labs-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'LOINC' and subtype= 'LAB' and is_standard = 1 group by 4,5,3,2
              )
order by domain_id, type, is_group, name, run_type;

-- MEASUREMENT - SNOMED - STANDARD - SQL-ORDER = 16
select * from (
                  select '01-sequential' run_type, domain_id, type, is_group, 'snomed-meas-std-name' name, count(is_group) sum_grp_count
                       , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'SNOMED' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '02-parallel' run_type, domain_id, type, is_group, 'snomed-meas-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'SNOMED'  and is_standard = 1 group by 4,5,3,2
                  union all
                  select '03-parallel-multi' run_type, domain_id, type, is_group, 'snomed-meas-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'SNOMED'  and is_standard = 1 group by 4,5,3,2
                  union all
                  select '10-original' run_type, domain_id, type, is_group, 'snomed-meas-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id = 'MEASUREMENT' and type = 'SNOMED' and is_standard = 1 group by 4,5,3,2
                  union all
                  select '20-std-src' run_type, domain_id, type, is_group, 'snomed-meas-std-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
...
select * from (
                  select '01-sequential' run_type, domain_id, type, is_group, 'proc-occur-snomed-src-name' name, count(is_group) sum_grp_count
                       , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where domain_id = 'PROCEDURE' and type = 'SNOMED' and is_standard = 0 group by 4,5,3,2
                  union all
                  select '02-parallel' run_type, domain_id, type, is_group, 'proc-occur-snomed-src-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where domain_id = 'PROCEDURE' and type = 'SNOMED' and is_standard = 0 group by 4,5,3,2
                  union all
                  select '03-parallel-multi' run_type, domain_id, type, is_group, 'proc-occur-snomed-src-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where domain_id = 'PROCEDURE' and type = 'SNOMED' and is_standard = 0 group by 4,5,3,2
                  union all
                  select '10-original' run_type, domain_id, type, is_group, 'proc-occur-snomed-src-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id = 'PROCEDURE' and type = 'SNOMED' and is_standard = 0 group by 4,5,3,2
                  union all
                  select '20-std-src' run_type, domain_id, type, is_group, 'proc-occur-snomed-src-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where domain_id = 'PROCEDURE' and type = 'SNOMED' and is_standard = 0 group by 4,5,3,2
              )
order by domain_id, type, is_group, name, run_type;
...
-- Row counts
select * from (
                  select '01-sequential' run_type, type, count(*) row_count from  `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where type like ('ICD%') group by 2
                  union all
                  select '02-parallel' run_type, type, count(*) row_count from  `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where type like ('ICD%') group by 2
                  union all
                  select '03-parallel-multi' run_type, type, count(*) row_count from  `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where type like ('ICD%') group by 2
                  union all
                  select '10-original' run_type, type, count(*) row_count from  `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where type like ('ICD%') group by 2
                  union all
                  select '20-std-src' run_type, type, count(*) row_count from  `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where type like ('ICD%') group by 2
              )
...
select seq.pc s, par.pc p, mult.pc m, ori.pc o, src.pc std from
   (select count(distinct path) pc from `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where domain_id='PROCEDURE' and type='CPT4' ) seq,
   (select count(distinct path) pc from `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where domain_id='PROCEDURE' and type='CPT4' ) par,
   (select count(distinct path) pc from `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where domain_id='PROCEDURE' and type='CPT4') mult,
   (select count(distinct path) pc from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id='PROCEDURE' and type='CPT4' ) ori,
   (select count(distinct path) pc from `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where domain_id='PROCEDURE' and type='CPT4' ) src
