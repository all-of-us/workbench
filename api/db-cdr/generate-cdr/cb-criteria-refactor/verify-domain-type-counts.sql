...
select * from (
                  ...
                  select '01-sequential' run_type, domain_id, type, is_standard, is_group,  'icd10-name' name, count(is_group) sum_grp_count
                       , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 6,5,4,3,2
                  union all
                  select '02-parallel' run_type, domain_id, type, is_standard, is_group,  'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 6,5,4,3,2
                  union all
                  select '03-parallel-multi' run_type, domain_id, type, is_standard, is_group,  'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 6,5,4,3,2
                  union all
                  select '10-original' run_type, domain_id, type, is_standard, is_group,  'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 6,5,4,3,2
                  union all
                  select '20-std-src' run_type, domain_id, type, is_standard, is_group,  'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 6,5,4,3,2
              )
order by domain_id, type, is_standard, is_group, name, run_type;

-- CONDITION_OCCURRENCE - SNOMED - STANDARD - SQL-ORDER = 13
select * from (
                  select '01-sequential' run_type, domain_id, type, is_group, 'icd10-name' name, count(is_group) sum_grp_count
                       , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySeq.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 4,5,3,2
                  union all
                  select '02-parallel' run_type, domain_id, type, is_group, 'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyPar.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 4,5,3,2
                  union all
                  select '03-parallel-multi' run_type, domain_id, type, is_group, 'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyMult.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 4,5,3,2
                  union all
                  select '10-original' run_type, domain_id, type, is_group, 'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 4,5,3,2
                  union all
                  select '20-std-src' run_type, domain_id, type, is_group, 'icd10-name' name, count(is_group) sum_grp_count
                          , sum(item_count) sum_item_count, sum(rollup_count) sum_rollup_count, sum(est_count) sum_est_count
                  from `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where domain_id = 'CONDITION' and type = 'SNOMED' group by 4,5,3,2
              )
order by domain_id, type, is_group, name, run_type;
...
--domain_id in ('CONDITION','PROCEDURE') and type in ('ICD9CM','ICD9Proc')

select id, parent_id, name, is_group, item_count, rollup_count, path from `all-of-us-ehr-dev.ChenchalDummySrc.cb_criteria` where domain_id='PROCEDURE' and type='ICD9Proc'
    except distinct
select id, parent_id,name, is_group, item_count, rollup_count, path from `all-of-us-ehr-dev.ChenchalDummyOri.cb_criteria` where domain_id='PROCEDURE' and type='ICD9Proc';
