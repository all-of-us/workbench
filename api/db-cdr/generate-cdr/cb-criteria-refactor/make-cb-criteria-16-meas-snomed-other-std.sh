...
# 16.0: #4186:  MEASUREMENT - SNOMED - STANDARD - add roots
  # cb_criteria: Uses : cb_criteria, concept, concept_ancestor, measurement
# 16.1: #4242:  MEASUREMENT - SNOMED - STANDARD - adding level 0
  #cb_criteria: Uses : cb_criteria, prep_snomed_rel_meas_in_data
# 16.2: #4295:  MEASUREMENT - SNOMED - STANDARD  - loop 9 times to add all items …
  #cb_criteria: Uses : cb_criteria, prep_snomed_rel_meas_in_data
# prep_ancestor_staging: #4356: Uses : cb_criteria
# prep_concept_ancestor: #4396: Uses : prep_ancestor_staging
# cb_criteria update counts: #4468: Uses : cb_criteria, measurement
# cb_criteria update parent counts: #4486: Uses : cb_criteria, prep_concept_ancestor, measurement
# add to make-cb-criteria-16-meas-snomed-hcpcs-std.sh
# ADD IN OTHER CODES NOT ALREADY CAPTURED (for SNOMED and HCPCS)
#6092 : MEASUREMENT - add other standard concepts (modified for SNOMED and HCPCS)
...
FROM
    (
        ...
        WHERE standard_concept = 'S'
            ...
            and vocabulary_id in ('SNOMED','HCPCS')
        GROUP BY 1,2,3,4
    )"
