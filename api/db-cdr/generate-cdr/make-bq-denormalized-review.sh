...
bq --project_id=$BQ_PROJECT rm -f $BQ_DATASET.cb_review_survey
bq --quiet --project_id=$BQ_PROJECT mk --schema=$schema_path/cb_review_survey.json --time_partitioning_type=DAY --clustering_fields person_id $BQ_DATASET.cb_review_survey
...
# Create bq tables we have json schema for
bq --project_id=$BQ_PROJECT rm -f $BQ_DATASET.cb_review_all_events
bq --quiet --project_id=$BQ_PROJECT mk --schema=$schema_path/cb_review_all_events.json --time_partitioning_type=DAY --clustering_fields person_id,domain $BQ_DATASET.cb_review_all_events
...
SELECT P.PERSON_ID,
    ...
    DATE_DIFF(t.DRUG_EXPOSURE_START_DATE,DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH), YEAR) - IF(EXTRACT(MONTH FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH))*100 + EXTRACT(DAY FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH)) > EXTRACT(MONTH FROM t.DRUG_EXPOSURE_START_DATE)*100 + EXTRACT(DAY FROM t.DRUG_EXPOSURE_START_DATE),1,0)  as AGE_AT_EVENT,
    T.NUM_MENTIONS,
    T.FIRST_MENTION,
    T.LAST_MENTION,
    T.QUANTITY as dose,
...
SELECT m.person_id,
    ...
    DATE_DIFF(m.measurement_date,DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH), YEAR) - IF(EXTRACT(MONTH FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH))*100 + EXTRACT(DAY FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH)) > EXTRACT(MONTH FROM m.measurement_date)*100 + EXTRACT(DAY FROM m.measurement_date),1,0) as age_at_event,
    case when c3.concept_name is null then '' else c3.concept_name end as visit_type,
    'LAB' as domain
...
SELECT P.PERSON_ID,
     ...
     DATE_DIFF(t.MEASUREMENT_DATE,DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH), YEAR) - IF(EXTRACT(MONTH FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH))*100 + EXTRACT(DAY FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH)) > EXTRACT(MONTH FROM t.MEASUREMENT_DATE)*100 + EXTRACT(DAY FROM t.MEASUREMENT_DATE),1,0) as AGE_AT_EVENT,
...
SELECT P.PERSON_ID,
     ...
     case when c2.CONCEPT_NAME is null then 'No matching concept' else c2.CONCEPT_NAME end as SOURCE_NAME,
     case when c2.CONCEPT_ID is null then 0 else c2.CONCEPT_ID end as SOURCE_CONCEPT_ID,
     DATE_DIFF(a.PROCEDURE_DATE,DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH), YEAR) - IF(EXTRACT(MONTH FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH))*100 + EXTRACT(DAY FROM DATE(p.YEAR_OF_BIRTH, p.MONTH_OF_BIRTH, p.DAY_OF_BIRTH)) > EXTRACT(MONTH FROM a.PROCEDURE_DATE)*100 + EXTRACT(DAY FROM a.PROCEDURE_DATE),1,0) as AGE_AT_EVENT,
     case when c3.concept_name is null then '' else c3.concept_name end as visit_type,
     'PROCEDURE' as domain
