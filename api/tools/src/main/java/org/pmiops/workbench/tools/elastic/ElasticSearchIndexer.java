 ...
 * following pipeline:
 *
 * - Create (and optionally replace) an existing index for this CDR.
 * - Query BigQuery to produce JSON documents: the output of the query is the exact format we define
 *   for Elasticsearch (using a repeated STRUCT for repeated nested documents).
 * - [Optional] Export these results to a temporary table, and export that to GCS. Slower, but
 *   necessary for larger datasets (e.g. the entire 1m participant Synthetic CDR).
 * - Bulk ingest the JSON documents into Elasticsearch (either inserts or updates).
 *
...
public final class ElasticSearchIndexer {
  ...
  private static Option queryProjectIdOpt = Option.builder()
      ...
      .desc("GCP Cloud Project ID to run the extraction query in. Does not necessarily need " +
  ...
  private static Option cdrVersionOpt = Option.builder()
      .longOpt("cdr-version")
      .desc("The CDR version identifier, e.g. 'synth_r_2018q1_1', see " +
          "https://docs.google.com/document/d/1W8DnEN7FnnPgGW6yrvGsdzLZhQrdOtTjvgdFUL6e4oc/edit")
      .required()
      .hasArg()
      .build();
  private static Option cdrBigQueryDatasetOpt = Option.builder()
      .longOpt("cdr-big-query-dataset")
      .desc("CDR BigQuery dataset ID (including the project), e.g. " +
          "'all-of-us-ehr-dev.synthetic_cdr20180606'")
      .required()
      .hasArg()
      .build();
  private static Option scratchBigQueryDatasetOpt = Option.builder()
      .longOpt("scratch-big-query-dataset")
      .desc("BigQuery dataset ID (including the project), for intermediate scratch exports e.g. " +
          "'all-of-us-ehr-dev.elastic-scratch'")
      .hasArg()
      .build();
  private static Option scratchGcsOpt = Option.builder()
      .longOpt("scratch-gcs-bucket")
      .desc("GCS bucket for intermediate JSON exports (without a gs://), e.g. " +
          "'all-of-us-workbench-test-elastic-exports'")
      .hasArg()
      .build();
  private static Option inverseProbOpt = Option.builder()
  ...
  private void createIndex(CommandLine opts) throws IOException, InterruptedException {
    ...
    if (opts.hasOption(scratchGcsOpt.getLongOpt())) {
      ...
      log.info(String.format("Exporting from CDR '%s' -> intermediate table '%s'", cdrDataset,
          scratchTableId));
      ...
      log.info(String
          .format("Exporting from intermediate table '%s' -> intermediate GCS file(s) '%s'",
      ...
      log.info(String
          .format("Converting intermediate GCS JSON file(s) '%s' -> Elasticsearch documents",
              gcsExportPath));
      Storage storage = StorageOptions.getDefaultInstance().getService();
      Iterator<String> lineIter = new CloudStorageShardedLineIterator(
  ...
  private String getPersonBigQuerySQL(String bqDataset, int inverseProb) throws IOException {
    String esPersonBQTemplate = Resources.toString(
