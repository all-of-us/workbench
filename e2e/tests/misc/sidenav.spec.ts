...
describe('Sidebar Navigation', () => {
  ...
  test('User can see the Contact Us form', async () => {
    ...
    const askQuestionAboutButton = await Button.forLabel({puppeteerPage: newIframe}, {text: HELP_DESK.ASK_QUESTION});
    expect(await askQuestionAboutButton.isVisible()).toBe(true);

    const reportConcernButton = await Button.forLabel({puppeteerPage: newIframe}, {text: HELP_DESK.REPORT_DATA_PRIVACY_CONCERN});
    expect(await reportConcernButton.isVisible()).toBe(true);
    await reportConcernButton.dispose();

    const tellAboutPublicationButton = await Button.forLabel({puppeteerPage: newIframe}, {text: HELP_DESK.TELL_US_ABOUT_PUBLICATION});
    expect(await tellAboutPublicationButton.isVisible()).toBe(true);
    await tellAboutPublicationButton.dispose();

    const requestBillingCreditsButton = await Button.forLabel({puppeteerPage: newIframe}, {text: HELP_DESK.REQUEST_BILLING_CREDITS});
  ...
  test('User can Sign Out', async () => {
    ...
    await page.waitForXPath('//*[text()="Redirect Notice"]');
