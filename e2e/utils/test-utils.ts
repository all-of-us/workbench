...
import {waitForText} from 'utils/wait-utils';
...
export async function newUserRegistrationSelfBypass(page: Page) {
  ...
  await Promise.race([
    ...
    Link.forLabel({puppeteerPage: page}, LABEL_ALIAS.SEE_ALL_WORKSPACES, {visible: true, timeout: 60000}),
  ]);
  ...
  await waitForText(page, {css: '[data-test-id="self-bypass"]'}, 'Bypass action is complete. Reload the page to continue.', 60000);
...
export async function performAction(
  ...
  case 'radiobutton':
    const radioELement = await RadioButton.forLabel({puppeteerPage: page}, identifier.textOption);
    await radioELement.select();
    break;
  case 'checkbox':
    const checkboxElement = await Checkbox.forLabel({puppeteerPage: page}, identifier.textOption);
    ...
    break;
  case 'textbox':
    const textboxElement = await Textbox.forLabel({puppeteerPage: page}, identifier.textOption);
    await textboxElement.type(value, {delay: 0});
    await textboxElement.tabKey();
    break;
  case 'textarea':
    const textareaElement = await Textarea.forLabel({puppeteerPage: page}, identifier.textOption);
