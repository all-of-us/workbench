...
/**
 * Summary: Helper functions when directly dealing with XPath selectors.
 */
 ...
 */
export async function waitForPageContainsText(page: Page, texts: string, timeOut: number = 30000): Promise<boolean> {

  try {
    const jsHandle = await page.waitForFunction(matchText => {
      const bodyText = document.querySelector('body').innerText;
      return bodyText.includes(matchText)
    }, {timeout: timeOut}, texts);
    await jsHandle.jsonValue();
    return true;
  } catch (e) {
    console.error(`Waiting for page contains "${texts}" text failed. ${e}`);
    throw e;
  }
}
 ...
 */
export async function visible(page: Page, cssSelector: string): Promise<boolean> {
  ...
  if (selector.css !== undefined) {
    try {
      ...
      const jsHandle = await page.waitForFunction((css, expText) => {
        ...
        return false;
      }, {timeout: timeOut}, selector.css, expectedText);
      await jsHandle.jsonValue();
      return true;
    } catch (e) {
      console.error(`Waiting for element matching CSS=${selector.css} contains "${expectedText}" text failed. ${e}`);
      throw e;
    }
    ...
    try {
      ...
      const jsHandle = await page.waitForFunction((xpath, expText) => {
        ...
        return false;
      }, {timeout: timeOut}, selector.xpath, expectedText);
      await jsHandle.jsonValue();
      return true;
    } catch (e) {
      console.error(`Waiting for element matching Xpath=${selector.xpath} contains "${expectedText}" text failed. ${e}`);
      throw e;
    }
  }
