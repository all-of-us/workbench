...
beforeEach(async () => {
  ...
  const includeUrl = (request: Request): Request | null => {
    const filters = ['google', 'content-security-index-report', 'analytics', 'status-alert', 'publicDetails'];
  ...
  const logResponse = async (request: Request): Promise<void> => {
    ...
    console.debug(
      '❗ Request finished: ' + `${request.response().status()} ${request.method()} ${request.url()}\n${responseText}`
    );
  };
