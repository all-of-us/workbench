...
def ensure_docker(cmd_name, args)
  unless Workbench::in_docker?
    exec *(%W{docker-compose run --rm public-ui ./project.rb #{cmd_name}} + args)
  end
end
...
def install_dependencies()
  common = Common.new
  common.run_inline %W{docker-compose run --rm public-ui yarn install}
end
...
def swagger_regen()
  ...
  common.run_inline %W{docker-compose run --rm public-ui yarn run codegen}
end
...
class DeployUI
  ...
  def run
    ...
    validate_options
    environment_names = {
      "all-of-us-public-ui-test" => "test",
      "all-of-us-public-ui-staging" => "staging",
      "all-of-us-public-ui-stable" => "stable",
    }
...
def dev_up(*args)
  ...
  common.status "    http://localhost:9877/debug.html\n"
  common.status "in a browser to view/run tests."

  common.run_inline %W{docker-compose run --rm --service-ports public-ui}
end
...
def run_linter()
  Common.new.run_inline %W{docker-compose run --rm public-ui yarn run lint}
end
...
def docker_run(cmd_name, args)
  Common.new.run_inline %W{docker-compose run --rm public-ui} + args
end
