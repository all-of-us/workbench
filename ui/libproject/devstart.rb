...
class DevStart
  ...
  def deploy_ui_js(cmd_name, args)
    DeployUI_withJS.new(cmd_name, args).run
  end
  ...
  def register_commands
    ...
    Common.register_command({
                              :invocation => "deploy-ui-js",
                              :description => "Deploys the UI to the specified cloud project using Javascript.",
                              :fn => ->(*args) { deploy_ui_js("deploy_ui_js", args) }
                            })
    Common.register_command({
                                :invocation => "build",
...
class DeployUI_withJS
  ...
  def run
    ...
    project = @opts.project
    version = @opts.version
    promote = @opts.promote ? 'true': 'false'
    account = @opts.account
    key_file = @opts.key_file
    dry_run = @opts.dry_run ? 'true' : 'false'

    node_cmd = ["node", "./libproject/buildAndDeploy.mjs"]
