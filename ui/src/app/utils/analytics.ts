...
export const AnalyticsTracker = {
  ...
  WorkspaceUpdatePrompt: {
    LooksGood: () =>
      triggerEvent(
        AnalyticsCategory.WORKSPACE_UPDATE_PROMPT,
        "Click 'Looks  Good'",
        'No Edits'
      ),
    UpdateWorkspace: () =>
  ...
  DatasetBuilder: {
    OpenCreatePage: () =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Open Create Page'),
    Create: () => triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Create'),
    Save: () => triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Save'),
    OpenEditPage: (suffix) =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Open Edit Page', suffix),
    Update: () => triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Update'),
    SeeCodePreview: () =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'See Code Preview'),
    OpenRenameModal: () =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Open Rename Modal'),
    Rename: () => triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Rename'),
    OpenExportModal: () =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Open Export Modal'),
    Export: (suffix) =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Export', suffix),
    OpenDeleteModal: () =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Open Delete Modal'),
    OpenGenomicExtractionModal: (suffix) =>
      triggerEvent(
        AnalyticsCategory.DATASET_BUILDER,
        'Open Genomic Extraction Modal',
        suffix
      ),
    Delete: () => triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'Delete'),
    ViewPreviewTable: () =>
      triggerEvent(AnalyticsCategory.DATASET_BUILDER, 'View Preview Table'),
  },
  Notebooks: {
    OpenCreateModal: () =>
      triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Open Create Modal'),
    Create: (suffix) =>
      triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Create', suffix),
    OpenRenameModal: () =>
      triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Open Rename Modal'),
    Rename: () => triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Rename'),
    Duplicate: () => triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Duplicate'),
    OpenCopyModal: () =>
      triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Open Copy Modal'),
    Copy: () => triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Copy'),
    OpenDeleteModal: () =>
      triggerEvent(AnalyticsCategory.NOTEBOOKS, 'Open Delete Modal'),
  ...
  Registration: {
    ...
    CreateAccount: () =>
      triggerEvent('Registration', 'Click create account', 'Create account'),
    TOS: () => triggerEvent('Registration', 'Click next', 'Accepted TOS'),
    InstitutionNotListed: () =>
      triggerEvent(
        'Registration',
        "Click 'don't see institution' link",
        'Institution not listed'
      ),
    InstitutionPage: () =>
      triggerEvent(
        'Registration',
        "Clicked 'Next' in step 1/3 in registration process",
        'Institution info completed'
      ),
    CreateAccountPage: () =>
      triggerEvent(
        'Registration',
        "Clicked 'Next' in step 2/3 in registration process",
        'Create Account page completed'
      ),
    DemographicSurvey: () =>
      triggerEvent(
        'Registration',
        "Clicked 'Next' in step 3/3 in registration process",
        'Demographic survey completed'
      ),
    TwoFactorAuth: () =>
      triggerEvent('Registration', "Clicked on '2FA' button", '2FA'),
    RegisteredTraining: () =>
      triggerEvent(
        'Registration',
        "Clicked on 'Ethics training' button",
        'Training'
      ),
    ControlledTraining: () =>
      triggerEvent(
        'Registration',
        "Clicked on 'Controlled tier training' button",
        'Training'
      ),
    ERACommons: () =>
      triggerEvent(
        'Registration',
        'Clicked on eRA commons button',
        'eRA commons'
      ),
    RasLoginGov: () =>
      triggerEvent(
        'Registration',
        'Clicked on RAS LoginGov linking button',
        'RAS LoginGov'
      ),
    EnterDUCC: () =>
      triggerEvent('Registration', 'Clicked in DUCC button', 'Entered DUCC'),
    AcceptDUCC: () =>
      triggerEvent('Registration', 'Clicked in DUCC button', 'Accepted DUCC'),
    TutorialVideo: () =>
      triggerEvent(
        'Home Page',
        'Clicked on a tutorial video',
        'Tutorial videos'
      ),
  },
  Sidebar: {
    OpenSidebar: (suffix) =>
      triggerEvent(AnalyticsCategory.SIDEBAR, 'Click', suffix),
    Search: (suffix) =>
      triggerEvent(
        AnalyticsCategory.HELP,
        'Search',
        `Help - Search - ${suffix}`
      ),
    ContactUs: (suffix) =>
      triggerEvent(
        AnalyticsCategory.HELP,
        'Click',
        `Help - Contact Us - ${suffix}`
      ),
    UserSupport: (suffix) =>
      triggerEvent(
        AnalyticsCategory.HELP,
        'Click',
        `Help - User Support - ${suffix}`
      ),
  },
  Footer: {
    Home: () => triggerEvent(AnalyticsCategory.FOOTER, 'Home', 'Home'),
    DataBrowser: () =>
      triggerEvent(AnalyticsCategory.FOOTER, 'Data Browser', 'Data Browser'),
    FeaturedWorkspaces: () =>
      triggerEvent(
    ...
    ResearchHub: () =>
      triggerEvent(AnalyticsCategory.FOOTER, 'Research Hub', 'Research Hub'),
    YourWorkspaces: () =>
      triggerEvent(
        AnalyticsCategory.FOOTER,
        'Your workspaces',
...
export function setLoggedInState(loggedIn: boolean) {
  gtagSet({
    [environment.gaLoggedInDimension]: loggedIn
 ...
 */
export function setInstitutionCategoryState(
  affiliation: VerifiedInstitutionalAffiliation
) {
  if (affiliation) {
    const category =
      OPERATIONAL_USER_INSTITUTION_SHORT_NAME ===
      affiliation.institutionShortName
        ? InstitutionCategoryState.OPERATIONS
        : InstitutionCategoryState.RESEARCHER;
    gtagSet({
      [environment.gaUserInstitutionCategoryDimension]: category,
    });
  } else {
    gtagSet({
      [environment.gaUserInstitutionCategoryDimension]:
        InstitutionCategoryState.NO_INSTITUTION,
    });
  }
}
...
export function initializeAnalytics() {
  gtag('js', new Date());
  gtagSet({
    [environment.gaUserAgentDimension]: window.navigator.userAgent.slice(
      0,
      100
    ),
    [environment.gaLoggedInDimension]: UserAuthState.LOGGED_OUT,
    [environment.gaUserInstitutionCategoryDimension]:
      InstitutionCategoryState.UNKNOWN,
  });
