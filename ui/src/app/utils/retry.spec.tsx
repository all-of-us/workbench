...
const functionStub = {
  ...
  failedFunction() {
    return Promise.resolve({ status: 504 }).then((response) => {
      throw response;
    });
  },
};
...
describe('IndexUtils', () => {
  it('should not retry if successful', async () => {
    const successfulFunctionSpy = jest.spyOn(
      functionStub,
      'successfulFunction'
    );
    await apiCallWithGatewayTimeoutRetries(() =>
      functionStub.successfulFunction()
    );
