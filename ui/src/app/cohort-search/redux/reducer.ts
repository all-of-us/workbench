...
import {
  ...
  RESET_STORE,
  RootAction,
  LOAD_CHART_RESULTS, BEGIN_INDIVIDUAL_PARTICIPANTS_CHART_REQUEST, LOAD_INDIVIDUAL_PARTICIPANTS_CHART_RESULTS,
} from './actions/types';
  ...
  (state: CohortSearchState = initialState, action: RootAction): CohortSearchState => {
    switch (action.type) {
      ...
      case BEGIN_INDIVIDUAL_PARTICIPANTS_CHART_REQUEST:
        return state
          .deleteIn(
            ['individualChartData', 'request', action.domain])
          .setIn(
      ...
      case LOAD_INDIVIDUAL_PARTICIPANTS_CHART_RESULTS:
        return state
          .setIn(
            ['individualChartData', 'chartsData', action.domain]
