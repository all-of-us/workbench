...
const styles = reactStyles({
  modal: {
    ...
    boxShadow: '0 1px 2px 2px rgba(0,0,0,.2)',
  },
  ...
  return (
    <ReactModal
      ...
      style={{
        ...
        content: props.contentStyleOverride || { ...styles.modal, width },
      }}
      ariaHideApp={false}
      {...props}
    >
...
export const AnimatedModal = ({ width = 450, ...props }) => {
  ...
  return (
    <Modal contentStyleOverride={{ margin: 'auto' }} {...props}>
      <div>
        <animated.div style={styleSpring}>{props.children}</animated.div>
      </div>
    </Modal>
  );
};
...
export const NotificationModal = () => {
  ...
  const {
    title = '',
    message = '',
    showBugReportLink: showBugReportLink = false,
    onDismiss = fp.noop,
  } = notification || {};
...
export const withErrorModal = fp.curry(
...
export const withSuccessModal = fp.curry(
  (notificationState: NotificationStore, wrappedFn) =>
    async (...args) => {
