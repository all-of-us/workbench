...
interface DurationInputProps {
  ...
  onChange: (value: number) => void; // milliseconds
  disabled?: boolean;
  style?: React.CSSProperties;
}

/**
 * A component for capturing duration values with separate inputs for days, hours, and minutes.
 * The value is represented in milliseconds, but displayed as days, hours, and minutes.
 */
...
export const DurationInput = ({
  ...
  const millisecondsToUnits = (milliseconds: number) => {
  ...
  const handleDaysChange = (days: number) => {
    const { hours, minutes } = millisecondsToUnits(value || 0);
    onChange(unitsToMilliseconds(days || 0, hours, minutes));
  };

  const handleHoursChange = (hours: number) => {
    const { days, minutes } = millisecondsToUnits(value || 0);
    onChange(unitsToMilliseconds(days, hours || 0, minutes));
  };

  const handleMinutesChange = (minutes: number) => {
    const { days, hours } = millisecondsToUnits(value || 0);
    onChange(unitsToMilliseconds(days, hours, minutes || 0));
  };

  const { days, hours, minutes } = millisecondsToUnits(value || 0);
