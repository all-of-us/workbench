...
const EMPTY_AUDIT_RESULT: GenericAuditQueryResult = {
  actions: [],
  logEntries: [],
  sourceId: 0,
  query: '',
...
const UserInput = ({
  ...
  return (
    <React.Fragment>
      ...
      />
      <TooltipTrigger
        content={`Retrieve the audit trail for selected ${auditSubjectType}.`}
      >
        <Button
          style={buttonStyle}
      ...
      >
        <TooltipTrigger
          content={'BigQuery Console page (use pmi-ops.org account)'}
        >
          <StyledExternalLink href={getBigQueryConsoleUrl()} target='_blank'>
            BigQuery Console
          </StyledExternalLink>
        </TooltipTrigger>
        &nbsp;|&nbsp;
        <TooltipTrigger
          content={`Admin Page for ${auditSubjectType} ${
            auditSubject || 'n/a'
          }`}
        >
          <StyledRouterLink
            ...
            style={
              auditSubject
                ? {}
                : { cursor: 'not-allowed', color: colors.disabled }
            }
      ...
      <TooltipTrigger
        content={
          'Download actual SQL query for BigQuery Action Audit table. Useful' +
...
export const AuditPageComponent = (props: AuditPageProps) => {
  ...
  const [queryResult, setQueryResult] =
    useState<GenericAuditQueryResult>(EMPTY_AUDIT_RESULT);
  const [displayNum, setDisplayNum] = useState(20);
  ...
  useEffect(() => {
    const getLogEntries = async () => {
      ...
      try {
        await queryAuditLog(initialAuditSubject).then(setQueryResult);
      } catch (e) {
        console.warn(
          `Error retrieving audit query results for ${initialAuditSubject}`
        );
      }
      setLoading(false);
    };
  ...
  return !loading ? (
    ...
    <div>
      Loading Audit for {auditSubjectType} {initialAuditSubject}...
    </div>
  );
};
