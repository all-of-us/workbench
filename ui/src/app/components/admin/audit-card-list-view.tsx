...
const PossibleLinkCell = (props: {
  ...
  if (isWorkspaceNamespace(targetType, targetProperty)) {
    return (
      <HideableLinkCell
        url={`/admin/workspace-audit/${value}`}
        content={value}
      />
    );
  } else {
...
const PropertyChangeListEntry = (props: {
  targetProperty?: string;
  ...
  return (
    isUserDrivenChange(newValue, previousValue) && (
      <React.Fragment>
...
const PropertyChangeListView = (props: { eventBundle: AuditEventBundle }) => {
  ...
  return propertyChanges.length > 0 &&
    fp.any((p: AuditTargetPropertyChange) =>
      isUserDrivenChange(p.newValue, p.previousValue)
    )(propertyChanges) ? (
    <div
      style={{
        marginTop: '0.25rem',
...
const TargetHeader = (props: { target: AuditTarget }) => {
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ color: colors.accent }}>{`${toTitleCase(
        target.targetType
      )} ${target.targetId || ''}`}</div>
      <div />
    </React.Fragment>
...
const AuditEventBundleHeaderView = (props: {
  ...
  return (
    <div>
      ...
      >
        <div style={{ color: colors.primary, margin: '5px' }}>
          {header.agent.agentType}
        </div>
        <div style={{ color: colors.success, margin: '5px' }}>
          {header.actionType}
        </div>
        <div style={{ color: colors.accent, margin: '5px' }}>
          {header.target.targetType}
        </div>
      </div>
      <div
        style={{
...
const AuditActionCard = (props: { action: AuditAction }) => {
  const { action } = props;
  const timeString = fp.flow(fp.get('actionTime'), moment, (m) =>
...
const ActionTypeFilter = (props: {
  ...
  const toggleSelectedAction = (actionType) => {
    updateFilter(
      actionType,
      !fp.get(`${actionType}.isActive`)(activeActionTypes)
    );
  };
...
export const AuditActionCardListView = (props: { actions: AuditAction[] }) => {
  ...
  const getActionTypes = (action: AuditAction) =>
    fp.flow(
  ...
  useEffect(() => {
    const actionTypeToActive = fp.flow(
      ...
      fp.toPairs,
      fp.map(([actionType, count]: [string, number]) => [
        actionType,
        {
          isActive: true,
  ...
  const updateFilterCallback = (actionType: string, isActive: boolean) => {
    setActiveActionTypes(
      fp.set([actionType, 'isActive'], isActive, activeActionTypes)
    );
  };
    ...
    fp.map(
      ([index, action]) =>
        shouldShowAction(action) && (
          <AuditActionCard key={index} action={action} />
        )
    ),
    fp.without([false])
  )(actions);
