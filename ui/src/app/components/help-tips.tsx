...
export class HelpTips extends React.Component<Props, State> {
  ...
  searchHelpTips(input: string) {
    ...
    const filteredContent = fp
      .values(JSON.parse(JSON.stringify(sidebarContent)))
      .reduce((acc, section) => {
  ...
  render() {
    ...
    const displayContent =
      filteredContent !== undefined
        ? filteredContent
        : sidebarContent[this.helpContentKey(this.props.pageKey)];
            ...
            <div key={s}>
              ...
              {section.content.map((content, c) => {
                return typeof content === 'string' ? (
                  <p key={c} style={styles.contentItem}>
                    {this.highlightMatches(content)}
                  </p>
                ) : (
                  <div key={c}>
                    <h4 style={styles.contentTitle}>
                      {this.highlightMatches(content.title)}
                    </h4>
                    {content.content.map((item, i) => (
                      <p key={i} style={styles.contentItem}>
