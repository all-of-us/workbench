import { z } from 'zod';
import { nonEmptyArray, optionalStringWithMaxLength, refinedObject, refineFields, requiredString, zodToValidateJS } from 'app/utils/zod-validators';
...
export const MAX_CHARACTERS_VALIDATION_MESSAGE = 'can have no more than %{count} characters.';
...
export const validateDemographicSurvey = (demographicSurvey: DemographicSurveyV2) => {
 ...
 **/ 
const requiredYesNoPreferNot = (msg: string = 'Required') => requiredString(msg);
const optionalMaxText = (prefix?: string, max = NONE_FULLY_DESCRIBE_MAX_LENGTH) =>  optionalStringWithMaxLength(max, prefix)

export const demographicSurveySchema = refinedObject<DemographicSurveyV2>((fields, ctx) => {
  const noop = undefined;
  const needYearOfBirthMsg = 'You must either fill in your year of birth or check the corresponding "Prefer not to answer" checkbox ';

  refineFields(fields, ctx, {
    ethnicCategories: nonEmptyArray(z.string(), "Ethnic categories can't be blank"),
    genderIdentities: nonEmptyArray(z.string(), "Gender identities can't be blank"),
    sexAtBirth: requiredString("Sex at birth can't be blank"),
    sexualOrientations: nonEmptyArray(z.string(), "Sexual orientations can't be blank"),
    yearOfBirth: fields.yearOfBirthPreferNot
      ? noop
      : z.number({required_error: needYearOfBirthMsg}).int()
        .min(minYear, { message: `Year of birth must be greater than or equal to ${minYear}` })
        .max(maxYear, { message: `Year of birth must be less than or equal to ${maxYear}` }),
    education: requiredString("Education can't be blank"),
    disadvantaged: requiredYesNoPreferNot("Disadvantaged can't be blank"),
    disabilityHearing: requiredYesNoPreferNot(" Difficulty hearing can't be blank"),
    disabilitySeeing: requiredYesNoPreferNot(" Difficulty seeing can't be blank"),
    disabilityConcentrating: requiredYesNoPreferNot(" Difficulty concentrating can't be blank"),
    disabilityWalking: requiredYesNoPreferNot(" Difficulty walking can't be blank"),
    disabilityDressing: requiredYesNoPreferNot(" Difficulty dressing can't be blank"),
    disabilityErrands: requiredYesNoPreferNot(" Difficulty doing errands can't be blank"),
    disabilityOtherText: optionalMaxText("Disability other text", OTHER_DISABILITY_MAX_LENGTH),
    ...
    ethnicityHispanicOtherText: optionalMaxText("Ethnicity hispanic other text"),
    ...
    surveyComments: optionalMaxText("Survey comments", SURVEY_COMMENTS_MAX_LENGTH),
  });
});
