...
interface RouteExtension extends Route {
  isPublic?: boolean;
}

declare type RoutesExtension = RouteExtension[];

const routes: RoutesExtension = [
  {
    path: 'login',
...
fp.forEach(route => {
  if (!route.isPublic) {
    route.canActivate = [SignInGuard];
    route.canActivateChild = [SignInGuard, RegistrationGuard];
