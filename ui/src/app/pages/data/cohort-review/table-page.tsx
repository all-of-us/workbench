...
export const ParticipantsTable = fp.flow(
  ...
  class extends React.Component<Props, State> {
    ...
    async componentDidMount() {
      ...
      if (!cohortReview) {
        promises.push(
          this.getParticipantStatuses().then(
            (response) => {
    ...
    goBack() {
      triggerEvent(
        EVENT_CATEGORY,
        'Click',
        'Back to cohort - Review Participant List'
      );
      const {
        match: {
          params: { ns, wsid, cid },
      ...
      this.props.navigateByUrl(`workspaces/${ns}/${wsid}/data/cohorts/build`, {
    ...
    showCohortDescription() {
      triggerEvent(
        'Cohort Description',
        'Click',
        'Cohort Description button - Review Participant List'
      );
      const { ns, wsid, cid } = this.props.match.params;
      this.props.navigate([
        'workspaces',
    ...
    filterTemplate(column: string) {
      ...
      const filtered =
        ...
        (column !== 'participantId' &&
          !filters[colType].includes('Select All'));
      let fl: any, ip: any;
      return (
        <span>
          {data && (
    ...
    render() {
      ...
      if (data !== null) {
        const lastRowOfPage =
          rows > data.length ? start + data.length : start + rows;
        pageReportTemplate = `${
          start + 1
        } - ${lastRowOfPage} of ${total} records `;
      }
      ...
      const columns = fields.map((col) => {
        const bodyStyle =
          col.field === 'participantId' ? idColumn : styles.columnBody;
        ...
        <div style={styles.review}>
          ...
          {!!cohortReview && (
            <React.Fragment>
              ...
              <div style={styles.description}>{cohortReview.description}</div>
              <DataTable
                style={styles.table}
                value={data}
                first={start}
