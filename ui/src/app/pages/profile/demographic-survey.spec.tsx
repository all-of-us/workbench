...
it('should load existing profile data', async () => {
  ...
  // Race
  expect(
    wrapper.find('CheckBox[data-test-id="checkbox-AIAN"]').prop('checked')
  ).toBeTruthy();
  expect(
    wrapper.find('CheckBox[data-test-id="checkbox-AA"]').prop('checked')
  ).toBeTruthy();
  expect(
    wrapper.find('CheckBox[data-test-id="checkbox-WHITE"]').prop('checked')
  ).toBeFalsy();

  // Gender identity
  expect(
    wrapper.find('CheckBox[data-test-id="checkbox-MAN"]').prop('checked')
  ).toBeTruthy();

  // Sex at birth
  expect(
    wrapper.find('CheckBox[data-test-id="checkbox-MALE"]').prop('checked')
  ).toBeTruthy();

  // LGBTQ
  // We use the .hostNodes() call to filter down to just the React component in the result set.
  expect(
    wrapper.find('[data-test-id="radio-lgbtq-yes"]').hostNodes().prop('checked')
  ).toBeTruthy();

  // Ethnicity
  expect(
    wrapper.find('[data-test-id="dropdown-ethnicity"]').first().prop('value')
  ).toEqual(Ethnicity.HISPANIC);
});
...
it('should handle error when submitting the survey', async () => {
  ...
  demographicSurvey.education =
    AccountCreationOptions.levelOfEducation[0].value;
  ...
  const wrapper = mount(
    <DemographicSurvey
      {...props}
      saveProfile={() => {
        throw new Response(JSON.stringify(errorResponseJson), { status: 412 });
      }}
    />
  );
