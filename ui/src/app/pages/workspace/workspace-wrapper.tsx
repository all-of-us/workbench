...
(class extends React.Component<{urlParams: any, profileState: {profile: Profile, reload: Function}}, State> {
  ...
  componentDidMount() {
    this.getTabPath(window.location.pathname.split('/').splice(-1).pop());
    this.getWorkspace();
  ...
  render() {
    ...
    return <React.Fragment>
      {displayNavBar &&
      <WorkspaceNavBarReact shareFunction={() => this.handleShareAction()}
          onTabSelect={(tp) => this.getTabPath(tp)} tabPath={tabPath}
