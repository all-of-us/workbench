...
import { EgressEvent, EgressEventsAdminApi, EgressEventStatus } from 'generated/fetch';
...
describe('AdminEgressAudit', () => {
  ...
  it('should allow event status update', async() => {
    ...
    wrapper.find('.p-dropdown-item').find({'aria-label': EgressEventStatus.VERIFIEDFALSEPOSITIVE}).simulate('click');
    wrapper.find({'data-test-id': 'save-egress-event'}).simulate('click');

    await waitForFakeTimersAndUpdate(wrapper);
    expect(eventsStub.events[0].status).toBe(EgressEventStatus.VERIFIEDFALSEPOSITIVE);
  });
});
