...
import { getOrderedAccessModules } from './admin-user-common';
...
describe('AdminUserProfile', () => {
  ...
  it('should render the titles of all expected access modules', async () => {
    ...
    expect(tableRows.length).toEqual(getOrderedAccessModules().length);

    // confirm that the orderedAccessModules are listed in order with expected title text
    expectModuleTitlesInOrder(getOrderedAccessModules(), tableRows);
  });
  ...
  test.each([
    ...
    async (
      ...
      // the previous test confirmed that the getOrderedAccessModules are in the expected order, so we can ref by index

      const { adminPageTitle } = getAccessModuleConfig(moduleName);
      const moduleRow = tableRows.at(
        getOrderedAccessModules().indexOf(moduleName)
      );
  ...
  it('should skip access modules when they are disabled in the environment', async () => {
    ...
    const expectedModules = getOrderedAccessModules().filter(
  ...
  test.each([
    ...
    async (
      ...
      // a previous test confirmed that the getOrderedAccessModules are in the expected order, so we can ref by index

      const eraRow = tableRows.at(
        getOrderedAccessModules().indexOf(AccessModule.ERACOMMONS)
      );
