...
describe('AdminReviewWorkspace', () => {
  ...
  it('should render and display a table', async () => {
    ...
    expect(
      wrapper.find('[data-test-id="reviewWorkspacesTable"]').length
    ).toBeGreaterThan(0);
  });
  ...
  it('should display workspaces for review', async () => {
    const wrapper = component();
    await waitOneTickAndUpdate(wrapper);
    expect(wrapper.find('[data-test-id="workspaceName"]').first().text()).toBe(
      workspaceStubs[0].name
    );
  });
  ...
  it('should no longer display the workspace once it has been approved or rejected', async () => {
    ...
    wrapper
      .find('[data-test-id="actionButtons"]')
      .first()
      .find('[data-test-id="approve"]')
      .first()
      .simulate('click');
    await waitOneTickAndUpdate(wrapper);
    expect(wrapper.find('[data-test-id="workspaceName"]').length).toBe(
      numWorkspaces - 1
    );
  });
});
