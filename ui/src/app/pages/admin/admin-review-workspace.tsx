 ...
 */
export const AdminReviewWorkspace = withUserProfile()(
  class extends React.Component<Props, State> {
    ...
    async approve(workspace: Workspace, approved: boolean) {
      ...
      try {
        await workspaceAdminApi().reviewWorkspace(
          workspace.namespace,
          workspace.id,
          { approved }
        );
      ...
      return workspaces.map((ws) => ({
        ...
        description: (
          <div>
            ...
            <br />
            {ws.researchPurpose.intendedStudy}
            <br />
            <i>Reason for choosing All of Us:</i>
            <br />
            {ws.researchPurpose.reasonForAllOfUs}
            <br />
            <i>Anticipated findings:</i>
            <br />
            {ws.researchPurpose.anticipatedFindings}
            <br />
          </div>
        ),
        actions: (
          <div>
            {this.state.reviewedWorkspace === ws ? (
              <Spinner size={50} />
            ) : (
              <React.Fragment>
                <Button
                  onClick={() => this.approve(ws, true)}
    ...
    render() {
      ...
      return (
        <div style={{ position: 'relative' }}>
          ...
          {contentLoaded ? (
            <DataTable
              value={this.convertWorkspaceToFields(workspaces)}
            ...
            >
              <Column
                ...
                sortable={true}
                data-test-id='workspaceName'
              />
              <Column
                field='creator'
                header='Workspace Author'
                headerStyle={{ width: '20%' }}
                sortable={true}
              />
              <Column
          ...
          {fetchingWorkspaceError && (
            <BugReportModal
              bugReportDescription='Could not fetch workspaces for approval'
          ...
          {reviewError && (
            <BugReportModal
              bugReportDescription={
                'Could not review workspace ' +
                reviewedWorkspace.namespace +
                '/' +
                reviewedWorkspace.name
              }
              onClose={() =>
                this.setState({ reviewError: false, reviewedWorkspace: null })
              }
            />
          )}
        </div>
