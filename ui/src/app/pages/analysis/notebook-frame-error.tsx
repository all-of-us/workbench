...
export const NotebookFrameError = ({
  errorMode = ErrorMode.ERROR,
  children,
}: Props) => {
  switch (errorMode) {
    case ErrorMode.INVALID:
      return (
        <div style={{ ...styles.previewMessageBase, ...styles.previewInvalid }}>
          {children}
        </div>
      );
    case ErrorMode.FORBIDDEN:
      return (
        <div
          style={{ ...styles.previewMessageBase, ...styles.previewForbidden }}
        >
          <ClrIcon
            style={{
...
export const SecuritySuspendedMessage = ({ error }: SuspendedMessageProps) => {
  ...
  const untilFull = moment(error.params.suspendedUntil).format(
    'MMMM Do YYYY, h:mm a'
  );
  return (
    <div data-test-id='security-suspended-msg'>
      <div>
        {until.isAfter(new Date()) ? (
          <>
            <b>
              Your analysis environment is suspended due to security egress
              concerns
            </b>
          ...
          <>
            Your analysis environment was temporarily suspended but is now
            available for use. Reload the page to continue.
          </>
        )}
        &nbsp;Please <b>check your contact email inbox</b> for follow-up and
        respond promptly to verify your activity.
      </div>
      ...
      <div>
        ...
        this{' '}
        <StyledExternalLink href={supportUrls.egressFaq} target='_blank'>
          support article
        </StyledExternalLink>{' '}
        or contact <SupportMailto /> with additional questions.
      </div>
    </div>
  );
};
