...
public interface DomainInfoDao extends CrudRepository<DomainInfo, Long> {
  ...
  /**
   * Returns domain metadata and concept counts for domains, matching standard concepts by name and
   * all concepts by code or concept ID. standardConceptCount is populated; allConceptCount and
  ...
  @Query(
      value =
          "select new org.pmiops.workbench.cdr.model.DomainInfo(\n"
              ...
              + "order by d.domainId")
  List<DomainInfo> findStandardOrCodeMatchConceptCounts(
      String matchExpression, String query, List<Long> conceptIds);
