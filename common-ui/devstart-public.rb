...
class CommonUiDevStart
  ...
  # @biopete -- taking a stab at commenting what these do for my own edification
...
class DeployUI
  ...
  def add_options
    ...
    @parser.on("--service-name [SERVICE_NAME]",
               "Provide service name other than default. " +
                   "Optional: deploys service other than default ") do |service_name|
      @opts.service_name = service_name
      @ops.app_yaml = "#{@opts.service_name}-app.yaml"
    end
  end
