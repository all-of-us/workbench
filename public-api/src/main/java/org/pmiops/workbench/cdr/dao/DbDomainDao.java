...
public interface DbDomainDao extends CrudRepository<DbDomain, Long> {
    ...
    List<DbDomain> findByDbTypeAndConceptIdNot(String db_type,Long concept_id);
        ...
        @Query(nativeQuery=true,value="select d.domain_id, d.domain_display, d.domain_desc, d.db_type, d.domain_route,d.concept_id, count(distinct c.concept_id) as count_value from db_domain d\n" +
                ...
                "and  ((match(c.concept_name) against(?1 in boolean mode) ) or (match(cs.concept_synonym_name) against(?1 in boolean mode) ) or c.concept_id=?1 or c.concept_code=?1 )\n" +
            ...
            "and (((match(q.concept_name) against(?1 in boolean mode) ) or (match(cs.concept_synonym_name) against(?1 in boolean mode) ) or q.concept_id=?1 or q.concept_code=?1) or (match(r.stratum_4) against(?1 in boolean mode)))\n" +
