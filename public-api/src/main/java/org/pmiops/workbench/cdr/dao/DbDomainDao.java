...
public interface DbDomainDao extends CrudRepository<DbDomain, Long> {
    ...
    @Query(nativeQuery=true,value="select d.domain_id, d.domain_display, d.domain_desc, d.db_type, d.domain_route,d.concept_id, count(distinct c.concept_id) as count_value from db_domain d\n" +
            "join concept c on d.domain_id = c.domain_id join concept_synonym cs on c.concept_id=cs.concept_id \n" +
            "where d.db_type = 'domain_filter' and (c.count_value > 0 or c.source_count_value > 0)and c.standard_concept = 'S'\n" +
            "and  ((match(c.concept_name) against(?1 in boolean mode) ) or (match(cs.concept_synonym_name) against(?1 in boolean mode) ))\n" +
            ...
            "join concept q on r.stratum_2 = q.concept_id join concept_synonym cs on q.concept_id=cs.concept_id \n" +
            "where d.db_type = 'survey' and r.analysis_id = 3110\n" +
            "and (((match(q.concept_name) against(?1 in boolean mode) ) or (match(cs.concept_synonym_name) against(?1 in boolean mode) )) or (match(r.stratum_4) against(?1 in boolean mode)))\n" +
