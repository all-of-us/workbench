...
public interface QuestionConceptDao extends CrudRepository<QuestionConcept, Long> {
    ...
    @Query(nativeQuery = true,value="select c.concept_id,c.concept_name,c.domain_id,c.vocabulary_id,c.concept_code,c.count_value,c.prevalence from survey_question_map sqm1 join concept c on c.concept_id=sqm1.question_concept_id where is_main=1 and (select count(distinct question_concept_id) from survey_question_map where path=sqm1.id and question_concept_id != sqm1.question_concept_id)>0")
    List<QuestionConcept> findSurveyMainQuestionIds();
}
